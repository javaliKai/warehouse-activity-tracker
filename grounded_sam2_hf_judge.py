...
from utils.judging_module import judge_frame, draw_judge_overlay
...
for frame_idx, segments in video_segments.items():
    ...
    # Judge module
    # -----------------------------
    judge_result = judge_frame(
        detections=detections,
        labels=labels,
        frame_idx=frame_idx,
        proximity_threshold_px=40.0,       # tune later
        footwear_overlap_threshold=0.20,   # tune later
    )
    ...
    # Warning overlay
    # -----------------------------
    annotated_frame = draw_judge_overlay(annotated_frame, judge_result)
    ...
    if judge_result["is_violation"]:
        print(
            f"[Frame {frame_idx}] "
            f"{judge_result['overall_risk_level']} "
            f"({judge_result['overall_risk_score']:.2f}) - "
            f"{judge_result['top_reason']}"
        )
